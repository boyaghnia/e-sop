                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @php
                                ...
                                // Menentukan halaman awal dan akhir
                                $startPage = max(1, $currentPage - 1); // Mundur 1 halaman dari halaman aktif
                                $endPage = min($totalPages, $currentPage + 1); // Maju 1 halaman dari halaman aktif
                            @endphp

                            @foreach ($element as $page => $url)
                                {{-- Hanya tampilkan halaman dalam rentang yang ditentukan --}}
                                @if ($page >= $startPage && $page <= $endPage)
                                    @if ($page == $currentPage)
                                        {{-- Halaman yang sedang aktif --}}
                                        <span aria-current="page">
                                            <span
                                                class="relative -ml-px inline-flex cursor-default items-center border border-gray-300 bg-gray-100 px-4 py-2 text-sm leading-5 font-medium text-black"
                                            >
                                                {{ $page }}
                                            </span>
                                        </span>
                                    @else
                                        {{-- Halaman lain yang tidak aktif --}}
                                        <a
                                            href="{{ $url }}"
